...
import { getModels, addHistoryEntry, getSettings } from '../utils/storage';
import { changeModel, prepareBatchChangeModel } from '../utils/api';
import { addToBatchQueue } from '../utils/batchQueue';
import GenerationOptions from './GenerationOptions';
import { getResolution } from '../utils/constants';

export default function WorkflowB({ onBack, onNavigate }) {
  const [images, setImages] = useState([]); // [{ file, base64, name, result, status, saved, savedPath, error }]
  ...
  async function handlePick() {
    ...
    const newImgs = await Promise.all(paths.map(async (p) => ({
      ...
      result: '', status: 'idle', saved: false, savedPath: '', error: ''
    })));
      ...
      <div className="screen-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <h1>👤 Change Model</h1>
            <p>Upload one or more product images and swap the model in batch</p>
          </div>
                    ...
                    {img.status === 'error' && (
                      <div style={{ padding: '6px 10px' }}>
                        <button className="btn btn-outline btn-sm" style={{ width: '100%' }}
                          onClick={() => setImages(prev => { const u = [...prev]; u[i] = { ...u[i], status: 'idle', result: '' }; return u; })}>
                          ↻ Retry
                        </button>
                      </div>
                    )}
