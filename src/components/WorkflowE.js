...
export default function WorkflowE({ onBack, onNavigate }) {
  ...
  const addingRef = useRef(false); // synchronous guard — useState is async and misses rapid double-clicks
  ...
  async function handleAddToBatch() {
    ...
    const vp = products.filter(p => p.images && p.images.length > 0 && p.name);
    if (vp.length === 0) { addingRef.current = false; return setError('Please add at least one product with an image and name.'); }
    if (!selectedModel) { addingRef.current = false; return setError('Please select a model.'); }
    if (selectedShots.length === 0 && !includeDetail) { addingRef.current = false; return setError('Please select at least one shot type.'); }
    setAdding(true);
  ...
  async function handleApproveOne(key, productName, shotType) {
    ...
    try {
      ...
      await addHistoryEntry({ workflow: 'E', productName, modelId: selectedModel?.id, backgroundId: selectedBg?.id, shotType, outputPath: filePath, imageData: item.base64 });
