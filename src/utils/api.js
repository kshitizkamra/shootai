...
async function buildShotPromptE(shotType, category, hasPose = false, t) {
  ...
  if (shotType === 'Styled') {
    ...
    return `${(t.garment_orientation||{})['Styled'] || ''} ${(t.e_styled||{}).garment_absolute_lock || ''} ${(t.global||{}).garment_shape_lock || ''} ${(t.e_styled||{}).garment_accessories || ''} ${identity} ${poseAction} ${(t.e_styled||{}).framing || ''} ${bgLock} ${(t.e_styled||{}).garment_fidelity || ''} ${(t.e_styled||{}).print_lock || ''} ${lighting} ${framingLock}`;
  }
