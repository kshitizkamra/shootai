...
const GARMENT_ORIENTATION = {
  'Front':  'GARMENT ORIENTATION — NON-NEGOTIABLE: The FRONT of the garment must face the camera. Model faces forward directly toward the camera. Do NOT show the back of the garment.',
  'Styled': 'GARMENT ORIENTATION: By default the FRONT of the garment must be visible. EXCEPTION: Only show the BACK of the garment if the shot instruction or global instruction explicitly requests a back-styled shot.',
  'Side':   'GARMENT ORIENTATION — NON-NEGOTIABLE: Show only the SIDE PROFILE of the garment — neither the full front nor the full back. Model faces to the right.',
  'Back':   'GARMENT ORIENTATION — NON-NEGOTIABLE: The BACK of the garment must face the camera. Model faces completely away from the camera.',
};
...
export async function generatePDPShot({ modelImageBase64, productImagesBase64, backgroundImageBase64, poseImageBase64, shotType, productName, modelBodyType, modelDescription, detailNote, globalInstruction, shotInstruction, quality, apiSize, resolution, skipGemini }) {
  ...
  const q = quality || settings.defaultQuality || 'high';
  const sz = apiSize || '1024x1536';
  let shotPrompt = (GARMENT_ORIENTATION[shotType] ? GARMENT_ORIENTATION[shotType] + ' ' : '') + (SHOT_PROMPTS[shotType] || SHOT_PROMPTS['Front']);
...
export async function prepareBatchPDPShot({ modelImageBase64, productImagesBase64, backgroundImageBase64, poseImageBase64, shotType, productName, modelBodyType, modelDescription, detailNote, globalInstruction, shotInstruction, quality, resolution, label, model: modelOverride }) {
  ...
  let shotPrompt = (GARMENT_ORIENTATION[shotType] ? GARMENT_ORIENTATION[shotType] + ' ' : '') + (SHOT_PROMPTS_BATCH[shotType] || SHOT_PROMPTS_BATCH['Front']);
  if (shotType === 'Detail Close-Up' && detailNote) {
    shotPrompt += ` CROP AREA: Show ONLY from ${detailNote} — frame the image tightly to this region. Do NOT show the full body. Do NOT show areas outside this crop zone.`;
  }
  const productCount = productImages.length;
  ...
  const poseLine = effectivePose
    ? `${poseIdx}. Pose reference — MANDATORY pose blueprint. Copy the exact joint angles, arm bend, hip shift, shoulder tilt, leg position, and weight distribution precisely. The person and clothing in this image are irrelevant — use ONLY the body pose as a blueprint.`
    : '';
  const prompt = `I am uploading ${images.length} reference images:\n1. MODEL reference — this is the ONLY person to appear in the output. Use her exact face, body structure, skin tone, hair, and ${modelBodyType || 'body type'}.\n${productLines}\n${bgLine}${poseLine ? '\n' + poseLine : ''}\n\nGenerate a photorealistic fashion photograph.\n\nCHARACTER: ONLY the woman from reference image 1. ${modelBodyType || 'Hourglass'} body type${modelDescription ? ', ' + modelDescription : ''}.\nGARMENT: GARMENT LENGTH — NON-NEGOTIABLE: The hemline must fall at the EXACT same point on the body as shown in the product reference image. Do NOT shorten or lengthen the garment under any circumstances. A midi dress stays midi. An ankle-length garment stays ankle-length. Copy the length precisely from the reference. Reproduce the exact garment from the product reference image(s). CRITICAL: Preserve the EXACT color, shade, and tone of the garment — do not saturate, brighten, or shift the hue. If the garment has a print or pattern, reproduce it exactly — same motifs, same colors, same scale, same density, same placement. Do NOT simplify, reinterpret, or alter the print in any way. Do NOT add, move, or reinterpret any design element — if smocking, elastic, pleats, or any detail is not present on the front of the garment, do NOT place it on the front. Each design feature must appear only where it actually exists on the garment. If the garment has embroidery, zari work, mirror work, sequins, threadwork, or any surface embellishment, reproduce them exactly — same motifs, same placement, same colors, same scale, same density. Do NOT simplify, omit, or alter any surface embellishment in any way. The garment's neckline, silhouette, and overall construction must be identical across all angles — do NOT alter any structural design element between shots. Product: ${productName || 'fashion item'}.\nFOOTWEAR: Do NOT copy or reproduce footwear from any reference image — ignore the shoes on the model in reference image 1 and ignore any footwear visible in the product reference images. Default to simple nude pointed-toe heels unless a different footwear is specified in the global instruction. Apply the same footwear consistently across every shot in this set.\nSETTING: ${backgroundImageBase64 ? `reproduce the EXACT background from reference image ${bgIdx} — the wall color, floor color, and environment must match pixel-perfectly: same hue, same saturation, same brightness. The background is a FIXED CONSTANT — do NOT recolor, relight, warm, cool, or shift it based on the model's pose or angle. Only the lighting ON THE MODEL adapts to the background — never the other way around. Ignore any background visible in the product reference images.` : 'clean professional white studio — pure white walls and pure white floor, no grey, no off-white, no colored surfaces. This exact white studio background must be identical across every shot. Do NOT use or be influenced by any background, floor color, or environment visible in the product reference images — those backgrounds must be completely ignored.'}.${effectivePose ? `\nPOSE: Copy the EXACT pose from reference image ${poseIdx} — mirror the precise joint angles, arm positions, hip tilt, shoulder angle, leg stance, weight distribution, and foot placement exactly as shown. This is NON-NEGOTIABLE. Do NOT simplify, straighten, or reinterpret the pose in any way. Do NOT default to a generic standing pose. Every body angle must match the reference precisely.` : ''}\n\n${shotPrompt}${shotInstruction ? `\nSPECIAL INSTRUCTION FOR THIS SHOT: ${shotInstruction}` : ''}${globalInstruction ? `\nGLOBAL INSTRUCTION (applies ONLY to styling, mood expression, and accessories — does NOT override model identity, garment, or background): ${globalInstruction}` : ''}\nSoft diffused studio lighting. Premium D2C fashion brand photography quality.\nNo text, no overlays, no watermarks.`;
  return {
    workflow: 'C',
...
const CATEGORY_ACTIONS = {
  ...
  topwear: {
    Front: 'Action: 3/4 shot focusing on the upper garment. HEAD — NON-NEGOTIABLE: The model\'s full head from crown to chin must be completely visible — do NOT crop the top of the head. Leave clear headroom above the crown. FRAMING — ABSOLUTE: This is a CROPPED UPPER-BODY SHOT. The frame MUST end at hip or mid-thigh level. Legs and feet are completely outside the frame — do NOT show them under any circumstances. Camera is zoomed in; the upper garment fills the frame as the hero. Do NOT pull back to show the full body. FOOTWEAR NOTE: Footwear is NOT visible in this shot — do not attempt to show shoes or legs.',
    Side:  'Action: 3/4 side shot, model facing right. HEAD — NON-NEGOTIABLE: The model\'s full head from crown to chin must be completely visible — do NOT crop the top of the head. Leave clear headroom above the crown. FRAMING — ABSOLUTE: This is a CROPPED UPPER-BODY SHOT. The frame MUST end at hip or mid-thigh level. Legs and feet are completely outside the frame — do NOT show them. Show the side silhouette of the upper garment clearly. Do NOT pull back to show the full body. FOOTWEAR NOTE: Footwear is NOT visible in this shot — do not attempt to show shoes or legs.',
    Back:  'Action: 3/4 back shot, model facing away from camera. HEAD — NON-NEGOTIABLE: The model\'s full head from crown to chin must be completely visible — do NOT crop the top of the head. Leave clear headroom above the crown. FRAMING — ABSOLUTE: This is a CROPPED UPPER-BODY SHOT. The frame MUST end at hip or mid-thigh level. Legs and feet are completely outside the frame — do NOT show them. Show the back of the upper garment clearly. Do NOT pull back to show the full body. FOOTWEAR NOTE: Footwear is NOT visible in this shot — do not attempt to show shoes or legs.',
  },
  bottomwear: {
...
function buildShotPromptE(shotType, category, hasPose = false) {
  ...
  const bgLock = 'BACKGROUND COLOR — NON-NEGOTIABLE: The wall color and floor color from the background reference image must appear EXACTLY as shown — same hue, same saturation, same brightness. Do NOT recolor, relight, warm, cool, or shift the background in any way based on the model\'s pose or angle. The background is a FIXED CONSTANT. Only the lighting on the model adapts to the background — never the other way around.';
  ...
  if (shotType === 'Styled') {
    ...
    return `${GARMENT_ORIENTATION['Styled']} GARMENT — ABSOLUTE LOCK: Reproduce the garment from the product reference images EXACTLY. Do NOT alter: color, shade, silhouette, neckline, straps, waist details (belt/tie/bow/elastic), skirt construction, hemline height, or any construction detail. The hemline must fall at the EXACT same point as in the product reference — a midi stays midi, an ankle-length garment stays ankle-length. Do NOT add any belt, tie, or waist accessory that is not on the product garment. Do NOT remove any belt, tie, or waist detail that IS on the product garment. The pose reference person wears a different outfit — ignore that outfit entirely. Use ONLY the product reference images for the garment. ${identity} ${poseAction} FULL BODY — entire figure visible. FRAMING: The model should occupy at least 65% of the frame height — medium to medium-wide shot. ${bgLock} GARMENT FIDELITY — CRITICAL: The garment must be reproduced EXACTLY from the product reference images — same color, same print, same pattern, same construction. Do NOT change the garment color, print scale, print density, motifs, embellishments, hemline, neckline, or silhouette in any way. The pose reference image contains a person wearing a different garment — ignore that garment completely and reproduce ONLY the product garment from the product reference images. PRINT/PATTERN — NON-NEGOTIABLE: The product reference images show multiple views of the garment (front, back, etc.). Each print or graphic is FIXED to a specific side of the garment — front prints stay on the front, back prints stay on the back. Do NOT transfer or mirror a graphic from the back onto the front or vice versa. For this front-facing shot, reproduce ONLY the print or graphic that is naturally visible from the front of the garment. If the back has a different or more prominent print, it must NOT appear in this shot. Do NOT rescale, simplify, alter, or replace the print. Do NOT add borders, embroidery, or embellishments that are not present in the product reference. ${lighting} ${framingLock}`;
  }
  if (shotType === 'Detail Close-Up') {
    return `Action: ZOOM INTO THE ACTUAL GARMENT on the model. ${identity} Do NOT reimagine, reconstruct, add, or exaggerate any garment detail. Do not over-emphasize seams or stitching beyond what is visible in the reference. Do NOT add smocking, gathering, shirring, elastic, ruffles, or any embellishment at the waist or any other area that is not clearly visible in the reference images — if the garment has a plain seam at the waist, keep it as a plain seam. CRITICAL: The garment color must exactly match the color in all reference images — do not shift, darken, or alter the hue in any way. FOOTWEAR NOTE: Do NOT alter the framing of this shot to show footwear — only include footwear if feet fall naturally within the detail area being shown. Do not zoom out or reframe to accommodate shoes. ${bgLock} ${lighting} ${framingLock}`;
  }
  ...
  return `${GARMENT_ORIENTATION[shotType] ? GARMENT_ORIENTATION[shotType] + ' ' : ''}${identity} ${action} ${bgLock} ${lighting} ${extraShadow} ${framingLock}`.trim();
}
...
export async function prepareBatchPDPShotE({ modelImageBase64, productImagesBase64, backgroundImageBase64, poseImageBase64, shotType, productName, category, modelBodyType, modelDescription, detailNote, globalInstruction, shotInstruction, quality, resolution, label, model: modelOverride, meta, _settings }) {
  ...
  const prompt = `I am uploading ${images.length} reference images:\n1. MODEL reference — this is the ONLY person to appear in the output. Use her exact face, body structure, skin tone, hair, and ${modelBodyType || 'body type'}.\n${productLines}\n${bgLine}${poseLine ? '\n' + poseLine : ''}\n\nGenerate a photorealistic fashion photograph.\n\nCHARACTER: ONLY the woman from reference image 1. ${modelBodyType || 'Hourglass'} body type${modelDescription ? ', ' + modelDescription : ''}.\nGARMENT: GARMENT LENGTH — NON-NEGOTIABLE: The hemline must fall at the EXACT same point on the body as shown in the product reference image. Do NOT shorten or lengthen the garment under any circumstances. A midi dress stays midi. An ankle-length garment stays ankle-length. Copy the length precisely from the reference. Reproduce the exact garment from the product reference image(s). CRITICAL: Preserve the EXACT color, shade, and tone of the garment — do not saturate, brighten, or shift the hue. If the garment has a print or pattern, reproduce it exactly — same motifs, same colors, same scale, same density, same placement. Do NOT simplify, reinterpret, or alter the print in any way. Do NOT add, move, or reinterpret any design element — if smocking, elastic, pleats, or any detail is not present on the front of the garment, do NOT place it on the front. Each design feature must appear only where it actually exists on the garment. If the garment has embroidery, zari work, mirror work, sequins, threadwork, or any surface embellishment, reproduce them exactly — same motifs, same placement, same colors, same scale, same density. Do NOT simplify, omit, or alter any surface embellishment in any way. The garment's neckline, silhouette, and overall construction must be identical across all angles — sleeve length, sleeve style, collar, cuffs, hemline, and all structural elements must be reproduced exactly as shown in the reference. Do NOT alter any construction detail between shots. PRINT/PATTERN SCALE — CRITICAL: Whatever pattern the garment has (stripes, checks, prints, motifs, or any repeating element), reproduce it at the EXACT same scale, width, spacing, and density as it appears in the reference images. Do NOT rescale, compress, reinterpret, or simplify the pattern in any way. Wide stripes stay wide. Large checks stay large. Bold motifs stay bold. The pattern colors must be copied exactly from the reference — do NOT substitute, brighten, saturate, or shift any hue (e.g. off-white ≠ white, blue-grey ≠ navy, ivory ≠ cream). COLLAR AND CONSTRUCTION — CRITICAL: The collar type, sleeve style, cuffs, buttons, hemline, and overall silhouette must be reproduced exactly from the reference. Do NOT change the collar to a different style — a shirt collar stays a shirt collar, a band collar stays a band collar. Copy every construction detail from the reference. SLITS AND VENTS — CRITICAL: If the garment has any side slits, back vents, front slits, or any hem openings, reproduce them exactly — same side (left/right), same depth, same position. Do NOT omit, close, or hide any slit or vent visible in the reference images. These must appear in every shot angle where that part of the garment is visible. STRIPE DIRECTION AND PANEL LAYOUT — CRITICAL: If the garment has directional stripe panels (mitered seams, chevron sections, diagonal panels, or panels running in different directions), reproduce the exact direction, angle, and panel layout from the reference. Do NOT simplify directional stripe panels into uniform stripes — a mitered chevron yoke stays a mitered chevron yoke, diagonal side panels stay diagonal. Product: ${productName || 'fashion item'}.\nFOOTWEAR: Do NOT copy or reproduce footwear from any reference image — ignore the shoes on the model in reference image 1 and ignore any footwear visible in the product reference images. Default to simple nude pointed-toe heels unless a different footwear is specified in the global instruction. Apply the same footwear consistently across every shot in this set.\nSETTING: ${backgroundImageBase64 ? `reproduce the EXACT background from reference image ${bgIdx} — the wall color, floor color, and environment must match pixel-perfectly: same hue, same saturation, same brightness. The background is a FIXED CONSTANT — do NOT recolor, relight, warm, cool, or shift it based on the model's pose or angle. Only the lighting ON THE MODEL adapts to the background — never the other way around. The model must look like she was physically photographed in that location — not composited. Edges between model and background must be photo-realistic. Ignore any background visible in the product reference images.` : 'clean professional white studio — pure white walls and pure white floor, no grey, no off-white, no colored surfaces. Identical across every shot. Do NOT be influenced by any background in the product reference images. Do NOT be influenced by any background visible in the pose reference image. The model must look naturally and evenly lit within this studio environment.'}.${effectivePose ? `\nPOSE: Replicate the body stance and posture from pose reference image ${poseIdx} — same arm position, weight distribution, and body language. Adapt this pose naturally to the required camera angle and framing for this shot. Do NOT default to a plain standing pose when a pose reference is provided.` : ''}\n\n${shotPrompt}${shotInstruction ? `\nSPECIAL INSTRUCTION FOR THIS SHOT: ${shotInstruction}` : ''}${globalInstruction ? `\nGLOBAL INSTRUCTION (applies ONLY to styling, mood expression, and accessories — does NOT override model identity, garment, or background): ${globalInstruction}` : ''}\nPremium D2C fashion brand photography quality.\nNo text, no overlays, no watermarks.`;
