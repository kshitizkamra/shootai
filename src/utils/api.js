...
async function buildShotPromptE(shotType, category, hasPose = false, t, lightingPreset = null) {
  ...
  if (shotType === 'Detail Close-Up') {
    ...
    return `${(t.e_detail_closeup||{}).action || ''} ${closeupIdentityNote} ${identity} ${(t.e_detail_closeup||{}).body || ''} ${bgLock} ${lighting} ${framingLock}`;
  }
...
FOOTWEAR: ${(t.global||{}).footwear_block||''}
SETTING: ${backgroundImageBase64 ? `reproduce the EXACT background from reference image ${bgIdx} — the wall color, floor color, and environment must match pixel-perfectly: same hue, same saturation, same brightness. The background is a FIXED CONSTANT — do NOT recolor, relight, warm, cool, or shift it based on the model's pose or angle. Only the lighting ON THE MODEL adapts to the background — never the other way around. Ignore any background visible in the product reference images. PHOTOGRAPHIC STYLE MATCHING — CRITICAL: Match the photographic rendering style, texture, and ambient light of the background exactly. The model must look like they were physically photographed in that location — not composited onto it. Adjust the AMBIENT LIGHT on the model to match the background scene — color temperature, shadow direction, and overall luminosity. CRITICAL EXCEPTION: Do NOT apply background color grading to the garment — the garment's color, hue, and saturation must remain EXACTLY as in the product reference images regardless of background tone. Only the model's skin, hair, and shadows may reflect the ambient color temperature. Edges between model and background must be photo-realistic, not cut-out or sharp-masked.` : 'clean professional white studio — pure white walls and pure white floor, no grey, no off-white, no colored surfaces. This exact white studio background must be identical across every shot. Do NOT use or be influenced by any background visible in the product reference images. The model must look naturally and evenly lit within this studio environment.'}.${effectivePose ? `\nPOSE: Replicate the body stance and posture from pose reference image ${poseIdx} — same arm position, weight distribution, and body language. Adapt this pose naturally to the required camera angle and framing for this shot. Do NOT default to a plain standing pose when a pose reference is provided.` : ''}
