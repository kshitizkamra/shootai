...
async function buildShotPromptE(shotType, category, hasPose = false, t) {
  ...
  if (shotType === 'Detail Close-Up') {
    ...
    return `${(t.e_detail_closeup||{}).action || ''} ${closeupIdentityNote} ${identity} ${(t.e_detail_closeup||{}).body || ''} ${bgLock} ${framingLock}`;
  }
  ...
  return `${(t.garment_orientation||{})[shotType] ? (t.garment_orientation||{})[shotType] + ' ' : ''}${identity} ${action} ${(t.global||{}).garment_shape_lock || ''} ${(t.global||{}).print_lock_angle || ''} ${bgLock} ${framingLock}`.trim();
}
